/**
 * Test 16: Take-Profit (Trigger Limit Order)
 *
 * 1. Open LONG 0.1 SOL at $150
 * 2. Place trigger SELL (take-profit) with triggerCondition=ABOVE, triggerPrice=$170
 * 3. Move oracle to $175 (above trigger)
 * 4. Call triggerOrder to fire the trigger
 * 5. Fill the triggered order
...
async function main() {
	console.log('\n=== Test 16: Take-Profit (Trigger Limit) ===');
	...
	if (!posAfterOpen || posAfterOpen.side !== 'LONG') {
		...
		printTestResult('16-take-profit', false);
		await cleanupClients(ctx.client, taker.client);
		return;
	}
	console.log(`  Position: ${posAfterOpen.side} ${posAfterOpen.baseAmountSol} SOL`);

	// ── Step 2: Place trigger SELL (take-profit) ──
	console.log('\n--- Step 2: Place take-profit trigger SELL at $170 ---');
	const triggerPrice = new BN(170).mul(PRICE_PRECISION).div(tickSize).mul(tickSize);
	const limitPrice = new BN(168).mul(PRICE_PRECISION).div(tickSize).mul(tickSize);
	console.log(`  Trigger price: ${triggerPrice.toString()} (condition: ABOVE)`);
	console.log(`  Limit price: ${limitPrice.toString()}`);

	const triggerParams = getTriggerLimitOrderParams({
	...
	// ── Step 3: Move oracle to $175 ──
	console.log('\n--- Step 3: Move oracle to $175 ---');
	await setupMarket(ctx.client, SOL_PERP_MARKET_INDEX, 175);
	await sleep(2000);

	// ── Step 4: Trigger the order ──
	console.log('\n--- Step 4: Trigger the take-profit ---');
	await taker.client.fetchAccounts();
	...
	const posAfterTP = getPosition(taker.client, SOL_PERP_MARKET_INDEX);
	const posReduced = !posAfterTP ||
		new BN(posAfterTP.baseAmount).abs().lt(new BN(posAfterOpen.baseAmount).abs());

	console.log('  Position after take-profit:');
	if (posAfterTP) {
		console.log(`    Side: ${posAfterTP.side}, Base: ${posAfterTP.baseAmountSol} SOL`);
	} else {
		console.log('    No position (closed by take-profit)');
	}
	...
	printTestResult('16-take-profit', allPassed);
